...
namespace CodeMinion.ApiGenerator.Cupy
{
    ...
    {
        private int _function_count;
        private readonly CodeGenerator _generator;

        private readonly HashSet<string> all_api_functions = new HashSet<string>();

        private readonly string BaseUrl = "https://docs.scipy.org/doc/Cupy-1.16.1/reference/";
        private DynamicApi ndarray_api;

        // use this to avoid duplicates
        private readonly HashSet<string> parsed_api_functions = new HashSet<string>();
        ...
        public ApiGenerator()
        {
            ...
            _generator = new CodeGenerator
            {
                ...
                NameSpace = "Cupy",
                StaticModuleName = "np",
                PythonModuleName = "Cupy",
                TestFilesPath = Path.Combine(test_dir, "Cupy.UnitTest"),
                Usings = { "using Cupy.Models;" },
                ToPythonConversions =
                {
                ...
                SpecialConversionGenerators =
                    { SpecialGenerators.ConvertArrayToNDarray, SpecialGenerators.ConvertDict },
                SharpToSharpConversions =
                {
                    SpecialGenerators.ArrayToNDarrayConversion
                },
                InitializationGenerators = { SpecialGenerators.InitCupyGenerator }
            };
        }
        ...
        public string Generate()
        {
            ...
            var array_creation_api = new StaticApi
            {
                ...
                ImplName = "Cupy", // name of the singleton that implements the static API behind the scenes
                PythonModule = "Cupy" // name of the Python module that the static api wraps 
            };
            _generator.StaticApis.Add(array_creation_api);
            ParseCupyApi(array_creation_api, "routines.array-creation.html");
            // ----------------------------------------------------
            // ndarray
            // ----------------------------------------------------
            ...
            var array_manipulation_api = new StaticApi
                { PartialName = "array_manipulation", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(array_manipulation_api);
            ParseCupyApi(array_manipulation_api, "routines.array-manipulation.html");
            // ----------------------------------------------------
            // dtype
            // ----------------------------------------------------
            var dtype_api = new StaticApi
                { PartialName = "dtype", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            ...
            var bitwise_api = new StaticApi
                { PartialName = "bitwise", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(bitwise_api);
            ParseCupyApi(bitwise_api, "routines.bitwise.html");
            // ----------------------------------------------------
            // String operations
            // ----------------------------------------------------
            var string_api = new StaticApi
                { PartialName = "string", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(string_api);
            ParseCupyApi(string_api, "routines.char.html");
            // ----------------------------------------------------
            // Datetime Support Functions
            // ----------------------------------------------------
            var datetime_api = new StaticApi
                { PartialName = "datetime", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(datetime_api);
            ParseCupyApi(datetime_api, "routines.datetime.html");
            // ----------------------------------------------------
            // Data type routines
            // ----------------------------------------------------
            var dtype_routines_api = new StaticApi
                { PartialName = "dtype.routines", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(dtype_routines_api);
            ParseCupyApi(dtype_routines_api, "routines.dtype.html");
            // ----------------------------------------------------
            // Optionally Scipy-accelerated routines (linalg, fft, ...)
            // ----------------------------------------------------
            var linalg_fft_api = new StaticApi
                { PartialName = "linalg_fft", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(linalg_fft_api);
            ParseCupyApi(linalg_fft_api, "routines.dual.html");
            // ----------------------------------------------------
            // Discrete Fourier Transform (Cupy.fft)
            // ----------------------------------------------------
            var fft_api = new StaticApi
                { PartialName = "fft", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(fft_api);
            ParseCupyApi(fft_api, "routines.fft.html");
            // ----------------------------------------------------
            // Financial functions
            // ----------------------------------------------------
            var financial_api = new StaticApi
                { PartialName = "financial", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(financial_api);
            ParseCupyApi(financial_api, "routines.financial.html");
            // ----------------------------------------------------
            // Indexing routines
            // ----------------------------------------------------
            var indexing_api = new StaticApi
                { PartialName = "indexing", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(indexing_api);
            ParseCupyApi(indexing_api, "routines.indexing.html");
            // ----------------------------------------------------
            // Input and output
            // ----------------------------------------------------
            var io_api = new StaticApi
                { PartialName = "io", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(io_api);
            ParseCupyApi(io_api, "routines.io.html");
            // ----------------------------------------------------
            // Linear Algebra
            // ----------------------------------------------------
            var linalg_api = new StaticApi
                { PartialName = "linalg", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(linalg_api);
            ParseCupyApi(linalg_api, "routines.linalg.html");
            // ----------------------------------------------------
            // Logic functions
            // ----------------------------------------------------
            var logic_api = new StaticApi
                { PartialName = "logic", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(logic_api);
            ParseCupyApi(logic_api, "routines.logic.html");
            // ----------------------------------------------------
            // Mathematical functions
            // ----------------------------------------------------
            var math_api = new StaticApi
                { PartialName = "math", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(math_api);
            ParseCupyApi(math_api, "routines.math.html");
            // ----------------------------------------------------
            // Padding Arrays
            // ----------------------------------------------------
            var padding_api = new StaticApi
                { PartialName = "padding", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(padding_api);
            ParseCupyApi(padding_api, "routines.padding.html");
            // ----------------------------------------------------
            // Random sampling
            // ----------------------------------------------------
            var random_api = new StaticApi
                { PartialName = "random", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(random_api);
            ParseCupyApi(random_api, "routines.random.html");
            // ----------------------------------------------------
            // Set routines
            // ----------------------------------------------------
            var set_api = new StaticApi
                { PartialName = "set", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(set_api);
            ParseCupyApi(set_api, "routines.set.html");
            // ----------------------------------------------------
            // Sorting, searching, and counting
            // ----------------------------------------------------
            var sorting_api = new StaticApi
                { PartialName = "sorting", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(sorting_api);
            ParseCupyApi(sorting_api, "routines.sort.html");
            // ----------------------------------------------------
            // Statistics
            // ----------------------------------------------------
            var staticstics_api = new StaticApi
                { PartialName = "staticstics", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(staticstics_api);
            ParseCupyApi(staticstics_api, "routines.statistics.html");
            // ----------------------------------------------------
            // Window functions
            // ----------------------------------------------------
            var window_api = new StaticApi
                { PartialName = "window", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            _generator.StaticApis.Add(window_api);
            ParseCupyApi(window_api, "routines.window.html");
            // ----------------------------------------------------
            // Other functions
            // ----------------------------------------------------
            var other_api = new StaticApi
                { PartialName = "other", StaticName = "np", ImplName = "Cupy", PythonModule = "Cupy" };
            ...
            // ----------------------------------------------------
            // generate Cupy
            // it is based on Python.Included and packs the Cupy wheel
            // ----------------------------------------------------
            ndarray_api.OutputPath = Path.Combine(src_dir, "Cupy/Models");
            _generator.ModelsPath = Path.Combine(src_dir, "Cupy/Models");
            _generator.StaticApiFilesPath = Path.Combine(src_dir, "Cupy");
            _generator.DynamicApiFilesPath = Path.Combine(src_dir, "Cupy");
            _generator.Generate();
            ApiStatistics();
            Console.WriteLine(
                $"Number of generated functions: {parsed_api_functions.Count} / {all_api_functions.Count}");
        ...
        private void ParseDtypeApi(StaticApi api)
        {
            ...
            foreach (var tr in doc.Doc.DocumentNode.Descendants("tr"))
            {
                ...
                _function_count++;
                api.Declarations.Add(new Property
                    { Name = span.InnerText, Description = td?.InnerText, Type = "Dtype", HasSetter = false });
            }
        }

        private void ParseCupyApi(StaticApi api, string link)
        {
            var docs = LoadDocs(link);
            var testfile = new TestFile { Name = $"{api.ImplName}_{api.PartialName}" };
            _generator.TestFiles.Add(testfile);
            foreach (var html_doc in docs) ParseCupyDocPage(api, link, html_doc, testfile);
        }
        ...
        private void ParseSinglePages(StaticApi api, params string[] links)
        {
            ...
            var testfile = new TestFile { Name = $"{api.ImplName}_{api.PartialName}" };
            _generator.TestFiles.Add(testfile);
            foreach (var html_doc in docs) ParseCupyDocPage(api, "single pages", html_doc, testfile);
        }

        private void ParseCupyDocPage(StaticApi api, string link, HtmlDoc html_doc, TestFile testfile)
        {
            var doc = html_doc.Doc;
            // declaration
            var h1 = doc.DocumentNode.Descendants("h1").FirstOrDefault();
            ...
            foreach (var d in InferOverloads(decl))
            {
                ...
                if (ndarray_api != null && d.Arguments.FirstOrDefault()?.Type == "NDarray" && class_name == "Cupy.")
                {
                    switch (decl.Name)
                    {
        ...
        private string ParseDescription(HtmlNode dd)
        {
            ...
            var desc = string.Join("\r\n\r\n",
                dd.ChildNodes.Where(n => n.Name == "p").Select(p => p.InnerText)
                    .TakeWhile(s => !s.StartsWith("Examples")));
            return desc;
        }
        ...
        {
            var dl = htmlDoc.Doc.DocumentNode.Descendants("dl")
                .FirstOrDefault(x => x.Attributes["class"]?.Value == "function");
            if (dl == null)
                return;
        ...
        private void PostProcess(Function decl)
        {
            ...
            if (decl.Returns.Any(a => a.Type == "T" || a.Type.Contains("<T>"))) decl.Generics = new[] { "T" };
            // split combined arguments: NDarray x, y => NDarray x, NDarray y
            for (var i = decl.Arguments.Count - 1; i >= 0; i--)
            {
            ...
            switch (decl.Name)
            {
                ...
                case "load":
                    ...
                    // allow_pickle was changed in Cupy version 1.16.3: Made default False in response to CVE-2019-6446.
        ...
        private IEnumerable<Function> InferOverloads(Function decl)
        {
            ...
            var overloads = new HashSet<Function> { decl };
            var i = -1;
            foreach (var arg in decl.Arguments)
            {
                i++;
        ...
        // special treatment for cp.arange which is a "monster"
        ...
        private HtmlDoc GetHtml(string relative_url)
        {
        ...
        {
            var doc = GetCupyReference();
            var li = doc.Doc.DocumentNode.DescendantsOfClass("li", "toctree-l1")
                .FirstOrDefault(x => x.InnerText.StartsWith("Cupy Reference"));
        ...
        private void CollectApiFunctions(HtmlNode li, Stack<string> stack)
        {
            ...
            if (a.InnerText.StartsWith("Cupy"))
        ...
        public HtmlDoc GetCupyReference()
        {
            var url = "https://docs.scipy.org/doc/Cupy/contents.html";
