...
namespace Cupy.Models
{
    ...
    ///     Adapted from Greg Hewgill's answer on Stackoverflow:
    ///     https://stackoverflow.com/questions/509211/understanding-slice-notation
    ///     Note: special IsIndex == true
    ///     It will pick only a single value at Start in this dimension effectively reducing the Shape of the sliced matrix by
    ///     1 dimension.
    ...
    public class Slice
    {
        ...
        {
            return Regex.Split(multi_slice_notation, @",\s*").Where(s => !string.IsNullOrWhiteSpace(s))
                .Select(token => new Slice(token)).ToArray();
        }
        ...
        {
            var inst = cp.self; // <--- this is important, to make sure PythonEngine is initialized!
